			...
			namespace details {
				...
				_FwdIt _Min_element_impl(const _ExPolicy&, _FwdIt _First, _FwdIt _Last, _Pr _Pred, _IterCat)
				{
					...
					if (_First != _Last)
					{
						...
						_Partitioner<_ExecutionPolicy>::_For_Each(_First, std::distance(_First, _Last),
							[_Pred, &_Combine, &_First](_FwdIt _Begin, size_t _Count) {
							auto _End = _Begin;
							std::advance(_End, _Count);

							_Begin = std::min_element(_Begin, _End, _Pred);

							bool _Exists;
							auto &_Found = _Combine.local(_Exists);
							if (!_Exists || _Pred(*_Begin, *_Found) || (!_Pred(*_Found, *_Begin) && std::distance(_First, _Begin) < std::distance(_First, _Found)))
								_Found = _Begin;
						});
				...
				{
					return _Min_element_impl(_Policy, _First, _Last, [_Pred](std::iterator_traits<_FwdIt>::reference _Val, std::iterator_traits<_FwdIt>::reference _Val2){
				...
				std::pair<_FwdIt, _FwdIt> _Minmax_element_impl(const _ExPolicy&, _FwdIt _First, _FwdIt _Last, _Pr _Pred, _IterCat)
				{
					...
					if (_First != _Last)
					{
						...
						_Partitioner<_ExecutionPolicy>::_For_Each(_First, std::distance(_First, _Last),
							...
							std::pair<_FwdIt, _FwdIt> _Found_local = std::minmax_element(_Begin, _End, _Pred);

							bool _Exists;
							auto &_Found = _Combine.local(_Exists);
							if (!_Exists)
								_Found = _Found_local;
							else _Found = _Minmax_helper(_First, _Found, _Found_local, _Pred);
						});
