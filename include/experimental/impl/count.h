				...
				typename std::iterator_traits<_InIt>::difference_type _Count_if_impl(const _ExPolicy&, _InIt _First, _InIt _Last, _Pr _Pred, _IterCat)
				{
					...
					if (_First != _Last) {
						...
						_Partitioner<_ExecutionPolicy>::_For_Each(_First, std::distance(_First, _Last),
							[_Pred, &_Combine](_InIt _Begin, size_t _Count){

							auto &_CountEl = _Combine.local();

							LoopHelper<_ExecutionPolicy, _InIt>::Loop(_Begin, _Count, [&_CountEl, &_Pred](const typename std::iterator_traits<_InIt>::reference _El){
								if (_Pred(_El))
									_CountEl++;
							});
