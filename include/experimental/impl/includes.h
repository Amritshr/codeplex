			...
			namespace details {
				...
				inline bool _Includes_impl(const _ExPolicy&, _InIt _First, _InIt _Last, _InIt2 _First2, _InIt2 _Last2, _Pr _Pred, _IterCat)
				{
					...
					if (_First != _Last) {
						...
						_Partitioner<_ExecutionPolicy>::_For_Each(_First2, _Size,
							[_Pred, _First, _Last, _First2, _Last2, &_Token](_InIt2 _Begin, size_t _Partition_count) {
							...
							if (_First2 != _Begin) {
								_Begin = std::upper_bound(_Begin, _End, *_Begin, _Pred); // Find the next element only in the chunk

								if (_Begin == _End)
									return;
							}

							auto _LoBound = std::lower_bound(_First, _Last, *_Begin, _Pred);
							if (_LoBound == _Last || _Pred(*_Begin, *_LoBound)) {
								_Token.cancel();
								return;
							}
							...
							if (_End != _Last2) {
								_HiBound = std::upper_bound(_LoBound, _Last, *_End, _Pred);
								_End = std::upper_bound(_End, _Last2, *_End, _Pred);
							}
							else _HiBound = _Last;

							if (!std::includes(_LoBound, _HiBound, _Begin, _End, _Pred))
								_Token.cancel();
						});
