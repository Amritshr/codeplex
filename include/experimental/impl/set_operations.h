		...
		namespace parallel {
			namespace details
			{
				...
				_RandItr3 _ParallelSetOperation(_RandItr1 _Begin1, size_t _Len1, _RandItr2 _Begin2, size_t _Len2, _RandItr3 _Output, size_t _ConcurrencyLevel, SetOp _SetOp, _CancPos _CalcPos, _Comp _Cmp)
				{
					...
					// _Step 1: filter
					_Partitioner<static_partitioner_tag>::_For_Each(_ChunkInfo.begin(), _ChunkInfo.size(), [&](std::vector<_SplitedChunk>::iterator _CurItr, size_t) {
						...
						if (_End1 < _Len1)
						{
							...
							if (!_Cmp(_Begin1[_Start1], _Begin1[_End1]))
								return;
							while (!_Cmp(_Begin1[_End1 - 1], _Begin1[_End1]))
								--_End1;
						}

						// Adjust head, until it reach the beginning
						while (_Start1 > 0 && !_Cmp(_Begin1[_Start1 - 1], _Begin1[_Start1]))
							--_Start1;

						// find a paired range
						size_t _Start2 = _Start1 == 0 ? 0 : std::lower_bound(_Begin2, _Begin2 + _Len2, _Begin1[_Start1], _Cmp) - _Begin2;
						size_t _End2 = _End1 == _Len1 ? _Len2 : std::upper_bound(_Begin2, _Begin2 + _Len2, _Begin1[_End1 - 1], _Cmp) - _Begin2;

						_CurItr->_Start = _CalcPos(_Start1, _Start2);
						auto _OutItr = _Buffer.begin() + _CurItr->_Start;
						_CurItr->_Len = _SetOp(_Begin1 + _Start1, _Begin1 + _End1, _Begin2 + _Start2, _Begin2 + _End2, _OutItr, _Cmp) - _OutItr;
					}, 1);
					...
					// _Step 3: move
					_Partitioner<static_partitioner_tag>::_For_Each(_ChunkInfo.begin(), _ChunkInfo.size(), [&](std::vector<_SplitedChunk>::iterator _CurItr, size_t) {
						std::copy(_Buffer.begin() + _CurItr->_Start, _Buffer.begin() + _CurItr->_Start + _CurItr->_Len, _Output + _CurItr->_Accumulated);
					}, 1);
				...
				typename _enable_if_parallel<_ExPolicy, _RandItr3>::type set_union_impl(const _ExPolicy &_Policy, _RandItr1 _Begin1, _RandItr1 _End1, _RandItr2 _Begin2, _RandItr2 _End2, _RandItr3 _Output, _Comp _Cmp, std::random_access_iterator_tag)
				{
					...
					{
						size_t _ConcurrencyLevel = std::thread::hardware_concurrency() * 2;
