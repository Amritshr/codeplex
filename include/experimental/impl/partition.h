		...
		namespace parallel {
			namespace details
			{
				...
				size_t _Parallel_partition_impl(Itr _Begin, size_t size, const Predicate &_Pred, size_t _ChunkSize, size_t _ConcurrencyLevel)
				{
					...
					// main parallel phase
					details::_Partitioner<static_partitioner_tag>::_For_Each(_LeftOver.begin(), _LeftOver.size(), [&](std::vector<std::pair<size_t, size_t>>::iterator _CurItr, size_t) {
						...
						for (;;)
						{
							while ((!_IsPairRangeEmpty(_Left) || !_IsPairRangeEmpty(_Left = _Helper._AcquireLeft())) && _Pred(_Begin[_Left.first]))
								++_Left.first;

							while ((!_IsPairRangeEmpty(_Right) || !_IsPairRangeEmpty(_Right = _Helper._AcquireRight())) && !_Pred(_Begin[_Right.second - 1]))
								--_Right.second;
				...
				_FwdIt _Partition_impl(const _ExPolicy&, _FwdIt _First, _FwdIt _Last, _Pr _Pred, _IterCat)
				{
					...
					const size_t _HdConc = (std::min)(static_cast<size_t>(std::thread::hardware_concurrency()), _Size / (_Chunk_size * 2));
					if (_HdConc <= 1)
						return std::partition(_First, _Last, _Pred);
					else
				...
				inline _BidIt _Stable_partition_impl(const _ExPolicy&, _BidIt _First, _BidIt _Last, _Pr _Pred, _IterCat)
				{
					...
					if (_First != _Last) {
						...
						return _Stable_partition_impl_helper(_First, _Last, _Size, _Pred, _Size == 1 ? 1 : std::thread::hardware_concurrency());
					}
				...
				inline std::pair<_OutIt, _OutIt2> _Partition_copy_impl(const _ExPolicy&, _InIt _First, _InIt _Last, _OutIt _Dest, _OutIt2 _Dest2, _Pr _Pred, _IterCat)
				{
					...
					if (_First != _Last) {
						auto _Size = std::distance(_First, _Last);
						std::vector<std::pair<difference_type, difference_type>> _Filter(_Size);

						return _Partitioner<copy_partitioner_tag>::_For_Each(
							make_composable_iterator(_First, std::begin(_Filter)), _Size, _Output_token(_Dest, _Dest2),
