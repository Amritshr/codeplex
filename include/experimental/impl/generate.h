	...
	namespace experimental {
		namespace parallel {
			namespace details {
				...
				inline void _Generate_n_impl(const sequential_execution_policy&, _OutIt _First, _Diff _Count, _Fn _Func, _IterCat)
				{
					_EXP_TRY
						std::generate_n(_First, _Count, _Func);
					_EXP_RETHROW
				}

				template <class _ExPolicy, class _OutIt, class _Diff, class _Fn, class _IterCat>
				inline void _Generate_n_impl(const _ExPolicy& _Policy, _OutIt _First, _Diff _Count, _Fn _Func, _IterCat _Cat)
				{
					_For_each_n_impl(_Policy, _First, _Count, [_Func](typename std::iterator_traits<_OutIt>::reference _El){
						_El = _Func();
					}, _Cat);
				}

				template <class _ExPolicy, class _OutIt, class _Diff, class _Fn>
				inline typename _enable_if_parallel<_ExPolicy, void>::type _Generate_n_impl(const _ExPolicy&, _OutIt _First, _Diff _Count, _Fn _Func, std::output_iterator_tag _Cat)
				{
					_Generate_n_impl(seq, _First, _Count, _Func, _Cat);
				}

				template <class _OutIt, class _Diff, class _Fn, class _IterCat>
				inline void _Generate_n_impl(const execution_policy& _Policy, _OutIt _First, _Diff _Count, _Fn _Func, _IterCat _Cat)
				{
			...
			template <class _ExPolicy, class _OutIt, class _Diff, class _Fn>
			inline typename details::_enable_if_policy<_ExPolicy, void>::type generate_n(_ExPolicy&& _Policy, _OutIt _First, _Diff _Count, _Fn _Func)
			{
				...
				details::_Generate_n_impl(_Policy, _First, _Count, _Func, std::_Iter_cat(_First));
			}
		}
	}
