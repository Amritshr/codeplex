			...
			namespace details {
				...
				inline bool _Is_partitioned_impl(const _ExPolicy&, _InIt _First, _InIt _Last, _Pr _Pred, _IterTag)
				{
					...
					if (_Size > 1) {
						...
						_Partitioner<_ExecutionPolicy>::_For_Each(_First, _Size - 1,
							[_Pred, &_Token](_InIt _Begin, size_t _Count) {
							_ASSERTE(_Count > 0); // Must process at least 1 element

							// To avoid warning C4800 forcing value to bool
							bool _Last_val = _Pred(*_Begin) ? true : false;
							LoopHelper<_ExecutionPolicy, _InIt>::Loop(++_Begin, _Count,
								[&_Token, &_Pred, &_Last_val](typename std::iterator_traits<_InIt>::reference _El){
								bool _Val = _Pred(_El) ? true : false;
