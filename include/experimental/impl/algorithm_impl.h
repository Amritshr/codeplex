...
#include "algorithm_scheduler_app.h"
#else
#error "WINAPI_FAMILY is not correctly defined"
#endif
#else // WINAPI_FAMILY
#include "algorithm_scheduler.h"
#endif // WINAPI_FAMILY
	...
	namespace experimental {
		namespace parallel {
			namespace details {
				...
				_EXP_IMPL void __cdecl set_thread_context(void*);
				_EXP_IMPL void* __cdecl get_thread_context();
...
#pragma warning(pop)
				...
				private:
					size_t _GetCombinableSize() const _NOEXCEPT
					{
						   ...
						   size_t _Size = std::thread::hardware_concurrency();
						   if (_Size != static_cast<size_t>(-1))
						   {
							   // ConcRT gave us the number of vprocs.
							   _Size *= 2;
							   int index;
							   for (index = 0; index < _Num_candidate_primes; ++index)
							   {
								   if (_Size < _Candidate_primes[index])
									   break;
							   }

							   if (index < _Num_candidate_primes)
								   _Size = _Candidate_primes[index];
							   else
								   _Size = _Candidate_primes[_Num_candidate_primes - 1];
						   }
						   else
						   {
							   // Choose a reasonable arbitrary (prime) size.
							   _Size = 11;
						   }
...
#pragma warning(pop) // C4316
				...
				class _Waitable_Threadpool_chore : public _Threadpool_chore
				{
				...
				public:
					...
					_Waitable_Threadpool_chore() : _ChoreSetCmpEvent(nullptr), _Counter(2) {}
					_Waitable_Threadpool_chore(const _Waitable_Threadpool_chore &) : _ChoreSetCmpEvent(nullptr), _Counter(2) {}
...
#pragma warning(disable: 4324)
				...
				class __declspec(align(64)) _Static_chore : public _Waitable_Threadpool_chore
				{
				...
				public:
					...
					_Static_chore(_It _First, size_t _S, const _Fn&_Fn) :
						_Begin(_First), _Size(_S), _Func(_Fn), _Exception(nullptr) {}
					...
					static void wait(_RangeCt &_Range)
					{
						_Waitable_Threadpool_chore::wait(_Range);
					...
					template <typename _PartTag> friend struct _Partitioner;
				};
#pragma warning(pop) // C4324
				...
				template <typename _It, typename _Fn>
				class _Semi_splittable_chore : public _Waitable_Threadpool_chore
				{
				...
				*/
				...
				template<>
				struct _Partitioner<static_partitioner_tag>
				{
				private:
					template<typename _Container, typename _FwdIt, typename _Fn>
					static _FwdIt _For_Each_impl(_Container& _Chores, _FwdIt _First, size_t _Count, const _Fn& _Func, size_t _Chunk_size = 0)
					{
						if (_Chunk_size == 0) {
						...
						{
							_Chores.emplace_back(_First, _Chunk_size, _Func);
							schedule_chore(&_Chores.back());
							_Count -= _Chunk_size;
							std::advance(_First, _Chunk_size);
						}

						_Chores.emplace_back(_First, _Count, _Func);
						_Chores.back().waitable_invoke();
				...
				public:
					...
					static _FwdIt _For_Each(_FwdIt _First, size_t _Count, const _Fn& _Func, size_t _Chunk_size = 0)
					{
						std::vector<_Static_chore<_FwdIt, _Fn>> _Chores;

						return _For_Each_impl(_Chores, _First, _Count, _Func, _Chunk_size);
					}

					template<typename _FwdIt, typename _Fn, typename _Cleanup_callback>
					static _FwdIt _For_each_with_cleanup(_FwdIt _First, size_t _Count, const _Fn& _Func, _Cleanup_callback _Cleanup, size_t _Chunk_size = 0)
					{
						std::vector<_Static_chore<_FwdIt, _Fn>> _Chores;
						...
						try {
							...
							for (auto _It = std::begin(_Chores); _It != std::end(_Chores); ++_It)
								_Cleanup(_It->_Begin, _It->_Size, _It->_Exception);
				...
				template<>
				struct _Partitioner<auto_partitioner_tag>
				{
					template<typename _FwdIt, typename _Fn>
					static _FwdIt _For_Each(_FwdIt _First, size_t _Count, const _Fn& _Func, size_t _Chunk_size = 0)
					{
						typedef _Static_chore<_FwdIt, _Fn> _ChoreType;
						...
						while (_Count > _Chunk_size)
						{
							...
							_Chores.emplace_back(_First, _Step, _Func);
							schedule_chore(&_Chores.back());
							_Count -= _Step;
...
#pragma warning(disable: 4324)
				...
				class __declspec(align(64)) _Copy_chore : public _Waitable_Threadpool_chore
				{
				protected:
...
#pragma warning(pop) // C4324
				...
				template<>
				struct _Partitioner<remove_partitioner_tag> :
				public _Partitioner<copy_partitioner_tag>
				{
					template<typename _FwdIt, typename _OutToken, typename _First_stage, typename _Second_stage>
					static _OutToken _For_Each(_FwdIt _First, size_t _Count, _OutToken _Dest, const _First_stage& _Stage1, const _Second_stage& _Stage2, size_t _Chunk_size = 0)
					{
				...
				*/
				...
				template<>
				struct _Partitioner<parallel_execution_policy> :
				public _Partitioner<auto_partitioner_tag>
				{
				};

				// vector_execution_policy defaults to self guided partitioner
				template<>
				struct _Partitioner<vector_execution_policy> :
				public _Partitioner<auto_partitioner_tag>
				{
				};
...
#pragma warning(pop)
				...
				struct _enable_if_parallel :
					...
					std::is_base_of<vector_execution_policy, typename std::decay<_ExPolicy>::type>::value,
					_Ty>
				{
				};
...
} // std::experimental::experimental::parallel::details
