...
namespace experimental
{
	...
	{
	//	if (i_f >= i_l) return (i_l); // Empty range passed, probably an error.
	...
	I scan_block(I f, I l, const Idx& tidx) restrict(amp)
	{
		...
		scan_thread(row_sums, row_sums + tidx.tile_dim0, tidx);
	...
	void scan_range(const AV1& reduced_res, const AV2& R, const Ext& t_ext)
	{
		...
		static constexpr int max_block_sz = t_ext.tile_dim0 * 8; // 8 elems to be serially reduced per lane.
	...
	I scan(I f, I l) restrict(cpu)
	{
		...
		static constexpr unsigned int thread_cnt = 2048u;
	...
	void scan(const AV& R) restrict(cpu) // Overload for keeping the data on the accelerator.
	{
		static constexpr unsigned int thread_cnt = 4096u;
		static constexpr unsigned int SIMD_w = 256u;
