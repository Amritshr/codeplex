...
namespace experimental
{
	...
	T accumulate_serial(I f, I l, T init, unsigned int offset = 1u) restrict(amp)
	{
		while (f < l) {
			init += *f;
			f += offset;
	...
	void accumulate_blocks(const AV1& R, const AV2& accumulated_blocks, const Ext& t_ext)
	{
		...
		parallel_for_each(t_ext, [=](const tiled_index<t_ext.tile_dim0>& tidx) restrict(amp) {
			...
			t_l_m[tidx.local[0]] = accumulate_serial(b_f + t_ext.size() + tidx.local[0], cend(R), *(b_f + tidx.local[0]), t_ext.size());
	...
	T accumulate_serial(I f, I l, T init, unsigned int offset = 1u, Op op = Op()) restrict(amp)
	{
		while (f < l) {
			init = op(init, *f);
			f += offset;
	...
	void accumulate_blocks(const AV1& R, const AV2& accumulated_blocks, const Ext& t_ext, Op op)
	{
		...
		parallel_for_each(t_ext, [=](const tiled_index<t_ext.tile_dim0>& tidx) restrict(amp) {
			...
			tile_static T t_l_m[tidx.tile_dim0];
			t_l_m[tidx.local[0]] = accumulate_serial(b_f + t_ext.size() + tidx.local[0], cend(R), *(b_f + tidx.local[0]), t_ext.size(), op);
